...
- Added authenticated billing status, Stripe-hosted Checkout creation, and short-lived Stripe Customer Portal creation. Organization billing changes require an admin or designated billing contact; individual freelancers retain authority over their own billing scope.
- Checkout supports the documented Founding Pro monthly/annual and Teams monthly offers when their server-side Stripe Price IDs are configured. First-time billing scopes receive the documented 14-day trial; an existing billing customer cannot repeatedly receive it. Teams annual remains unpriced and unavailable rather than inventing an offer.
- Checkout enables automatic tax calculation, supported tax-ID collection, required billing-address collection, and location-dependent VAT disclosure. The final tax treatment, Stripe Tax registrations, invoice wording, refund policy, and consumer terms still require qualified Malta/EU tax and legal approval before live charges.
- Subscription entitlements change only from timestamp-checked, idempotently claimed Stripe webhooks verified against the unmodified raw request body. Subscription lifecycle, cancellation-at-period-end, renewal date, and invoice-payment failures are persisted. Card details and payment credentials are never stored by NotchMe.
- Signed metadata is not sufficient by itself: webhook processing verifies that the referenced user belongs to the claimed organization and that the subscription Price ID equals the server-configured product/interval before applying a plan. Active and trialing subscriptions grant access; past-due status is surfaced while access remains during recovery; canceled, unpaid, incomplete-expired, and paused states resolve to Free.
- Hosted redirect URLs are allow-listed to the exact Stripe Checkout and Billing Portal hosts. Application return URLs require HTTPS except for explicit loopback development hosts. Checkout remains disabled unless the secret key, webhook secret, and selected Price ID are configured.
