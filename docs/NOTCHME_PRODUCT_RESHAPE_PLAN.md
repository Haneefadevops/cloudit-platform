...
- Added Free (`€0`), Founding Pro (`€10/month`), and Teams (`€39/month`) presentation cards. Their public CTAs open registration; authenticated billing was subsequently implemented in Phase 6B and remains configuration-gated. Feature lists are limited to exposed application/plan capabilities.
...
#### Phase 6B implementation checkpoint: subscription billing foundation

- Removed the development `POST /v2/billing/upgrade` behavior and its browser mutation. A user can no longer grant themselves a paid plan through an application request.
- Added authenticated billing status, Lemon Squeezy-hosted Checkout creation, and fresh signed Customer Portal links. Organization billing changes require an admin or designated billing contact; individual freelancers retain authority over their own billing scope. The earlier Stripe adapter and provider-specific webhook route remain inactive compatibility code for a possible future supported-country entity.
- Checkout supports the documented Founding Pro monthly/annual and Teams monthly offers when their server-side Lemon Squeezy Variant IDs are configured. First-time billing scopes receive the product-configured 14-day trial; a previously known billing scope has the trial explicitly skipped. Teams annual remains unpriced and unavailable rather than inventing an offer.
- Lemon Squeezy is the merchant of record for customer checkout, invoicing, payment processing, and applicable sales-tax/VAT collection and remittance. This does not replace NotchMe's Sri Lankan company accounting, income-tax obligations, privacy duties, refund policy, consumer terms, or qualified legal and tax advice before live charges.
- Subscription entitlements change only from idempotently claimed Lemon Squeezy webhooks whose HMAC is verified against the unmodified raw body. Store, test/live mode, authenticated scope, and exact Variant ID are verified before applying a plan. Card details and payment credentials are never stored by NotchMe.
- Normalized lifecycle behavior preserves access for trials, active subscriptions, paused collection, payment recovery, and cancellation grace periods. An expired subscription removes paid access. Renewal/end dates and cancellation-at-period-end are persisted from provider facts.
- Hosted redirects are allow-listed to HTTPS Lemon Squeezy domains. Application return URLs require HTTPS except for explicit loopback development hosts. Checkout remains disabled unless the API key, signing secret, store ID, explicit test/live mode, and selected Variant ID are configured.
- Migration `0025_lemon_squeezy_billing.sql` expands the provider constraint without deleting or rewriting legacy Stripe billing rows. `billing_subscriptions` continues to contain lifecycle identifiers/state and `billing_webhook_events` provides delivery idempotency; neither table has payment-card fields.
- Live-mode completion remains an external release gate: obtain Lemon Squeezy merchant approval, configure the EUR subscription variants and 14-day trial, configure the Customer Portal and store identity, register `/api/v2/billing/webhooks/lemon-squeezy`, apply migration `0025_lemon_squeezy_billing.sql`, and exercise test-mode then live checkout, invoice, renewal, cancellation, expiry, refund, dispute, and failed-payment behavior. Existing paid development rows require a manual entitlement audit and are not automatically revoked.
...
- Added public pre-launch Privacy, Terms, Cookie, and Security pages and linked them from the public footer. The text reflects current behavior, including optional AI processing, Lemon Squeezy-hosted merchant-of-record billing, essential sessions, account controls, absence of advertising cookies, and remaining production safeguards; it does not invent a legal entity, address, jurisdiction, or contact email.
...
- The full NotchMe API suite passes with 24 suites and 87 tests, and the API lint/build pass on the restored coherent Nest 10 runtime. Authentication, scope boundaries, booking lifecycle, recaps, AI privacy controls, Lemon Squeezy billing, retained Stripe compatibility, account control, analytics, and migration structures are represented in that suite.
...
- Production PostgreSQL execution for migrations `0020`–`0025`, live Lemon Squeezy/Resend/OpenAI checks, DNS/TLS, legal approval, backups/restore, monitoring, and incident ownership remain external release gates. The operational sequence is documented in `docs/NOTCHME_RELEASE_AND_PILOT_RUNBOOK.md`.
