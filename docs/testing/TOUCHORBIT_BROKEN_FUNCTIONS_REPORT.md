...
- **Last tested:** 2026-07-15
- **Notes:** The employee page resolves identity through `/employees/me`, lists and creates corrections through the local API, and surfaces API errors. The deployed EF5.1 retest passed: create returned 201 and the refreshed employee-scoped list displayed the new pending correction.
...
- **Last tested:** 2026-07-14
- **Notes:** Attendance remains on the local reports API. Phase 9 now normalizes PostgreSQL numeric fields before table/stat rendering and treats malformed data payloads as an empty result. API and admin builds pass; deployment E2E is pending.
...
- **Last tested:** 2026-07-14
- **Notes:** Leave remains on the local reports API and uses the shared row normalization/empty-result boundary. API and admin builds pass; deployment E2E is pending.
...
- **Last tested:** 2026-07-18 local build
- **Notes:** Phase 8 payroll coercion is now reinforced by the shared Phase 9 report response normalization. API and admin builds pass; deployment E2E is pending.
...
- **Last tested:** 2026-07-14
- **Notes:** The roster page now calls `GET /api/reports/adherence` on the local API, backed by the organization-scoped roster query, instead of the failing internal Supabase RPC route. Numeric results are normalized before render. API and admin builds pass; deployment E2E is pending.
...
- **Last tested:** 2026-07-14
- **Notes:** Overtime generation now uses an organization-scoped local SQL report endpoint with date, department, and employee filters. API and admin builds pass; deployment E2E is pending.
...
- **Last tested:** 2026-07-14
- **Notes:** Late-arrival generation now uses a local SQL endpoint that applies the organization grace period and returns normalized severity/repeat-count rows. API and admin builds pass; deployment E2E is pending.
...
- **Last tested:** 2026-07-14
- **Notes:** Expense generation now uses an organization-scoped local SQL endpoint with date, department, employee, and status filters. API and admin builds pass; deployment E2E is pending.
...
| 2026-07-20 | Local build           | `npm.cmd run build --workspace=apps/touchorbit-api`                                                                                    | Passed  | Build output                                                                                                                                                                                                                                                                                                                                                                                           | Phase 9 report endpoints compile: attendance, leave, payroll, adherence, overtime, late arrivals, and expense.                                                                                                                 |
