# Kimi Claw Instructions — CloudIT Platform Sprints 3-7

> **HOW TO USE THIS FILE:**
> 1. Kimi Claw reads this file at the start of every session.
> 2. Find the **CURRENT TASK** section below — it tells you exactly what to do.
> 3. After Kimi Claw finishes a task, it will update this file automatically.
> 4. Kimi Claw will commit and push the updated file to GitHub.

---

## CURRENT TASK TRACKER

**Last Updated:** 2026-06-28 14:32
**Current Sprint:** Sprint 4
**Current Task:** Task 4.7 — Build Hospitality Dashboard & Property Management
**Status:** NOT STARTED
**Next Task:** Task 4.8 — Build Reservations, Calendar & Check-in/out UI
...
```
You are building the CloudIT Platform. We are in Sprint 3, Task 3.1.
...
6. `Toast` — using a simple toast context/provider. Create `ToastProvider`, `useToast`, and `Toast` component.
...
11. `Calendar` — placeholder component that renders a simple grid.
...
6. On success: store JWT in localStorage (with a note to switch to httpOnly cookie later)
...
1. Test build with `npm run build` from `apps/platform-web`.
...
2. Table showing all users with columns:
   - Name, Email, Organization, Role, Status, Created At, Actions (Edit, Delete)
...
**API Integration**:
1. Fetch users from `GET /api/users`
...
7. Create Dockerfile for hospitality-api (multi-stage, similar to platform-api)
8. Update docker-compose to add hospitality-api service
...
1. Model: TaxRate with fields: name, rate (Decimal), type (percentage, fixed), isActive, isDefault, organizationId
...
1. Model: Invoice with fields: invoiceNumber, reservationId, propertyId, guestId, issueDate, dueDate, subtotal, taxBreakdown (JSON), totalAmount, paidAmount, status, notes
...
1. Create a simple HTML template for invoice
2. Endpoint: GET /invoices/:id/preview — returns JSON with formatted invoice data
...
1. Occupancy Report: GET /reports/occupancy?propertyId=X&startDate=Y&endDate=Z
   - Returns: totalRooms, occupiedRooms, occupancyRate, revenue, by date
2. Revenue Report: GET /reports/revenue?propertyId=X&startDate=Y&endDate=Z
   - Returns: totalRevenue, taxBreakdown, by room type, by payment status
3. Guest Report: GET /reports/guests?propertyId=X&startDate=Y&endDate=Z
   - Returns: totalGuests, newGuests, returningGuests, topNationalities
4. Reservation Report: GET /reports/reservations?propertyId=X&startDate=Y&endDate=Z
...
- Cache results in Redis for 1 hour (optional)
...
7. Create Dockerfile for hospitality-web
...
1. Stats cards: Today's Check-ins, Today's Check-outs, Occupancy Rate, Today's Revenue
...
**API Integration**:
- Connect to hospitality-api endpoints
...
1. Table: Reservation #, Guest, Room, Check-in, Check-out, Status, Total, Actions
...
5. New Reservation button → modal form:
   ...
   - Room select (filtered by availability)
   - Check-in/out dates
...
1. Month view calendar (use react-calendar or build simple)
2. Each date shows: check-ins (green), check-outs (red), occupancy percentage
...
2. Show room charges summary and tax breakdown
3. Generate invoice button
4. Check-out button → confirmation → updates status
...
3. View Invoice button → modal showing:
   - Property details, Guest details
...
2. Define event types as constants:
   ...
   - booking.checked_in, booking.checked_out, invoice.generated
...
1. Emit `booking.created` when reservation is created
...
1. Create `booking-notification.json` — n8n workflow that:
   - Trigger: Webhook node listening for `booking.created`
   - Action: Send email notification (placeholder)
   - Action: Log to Google Sheets or internal DB (placeholder)
   - Include README on how to import into n8n
2. Create `checkin-reminder.json` — workflow that:
   - Trigger: Cron (daily at 9 AM)
   - Action: Query hospitality API for today's check-ins
   - Action: Send WhatsApp message placeholder

**AI Module Placeholder** (`apps/platform-api/src/ai/`):
1. Create AiService with methods:
   - generateResponse(prompt): returns placeholder
   - summarizeText(text): returns placeholder
   - analyzeSentiment(text): returns placeholder
2. Create AiController with endpoints:
   - POST /ai/generate
   - POST /ai/summarize
3. Add AI_PROVIDER, AI_API_KEY to .env.example

**WhatsApp Integration Placeholder** (`apps/platform-api/src/integrations/`):
1. Create WhatsAppService with methods:
   - sendMessage(phone, message): logs to console
   - sendTemplate(phone, templateId, data): placeholder
2. Create WhatsAppController with webhook verification endpoint
...
1. Table showing event logs from platform-api:
   - Event Type, Payload (collapsible JSON), Status, Webhook URL, Response Status, Created At
2. Filter by event type and status
3. Retry failed events button
4. Pagination

**Integration Settings Page** (`apps/platform-web/src/app/dashboard/admin/integrations/page.tsx`):
1. n8n webhook URL and secret configuration
2. AI provider selection (OpenAI, Anthropic, Local, None)
3. AI API key input (masked)
4. WhatsApp API key and phone number ID inputs
5. Save button → PATCH to platform-api settings endpoint
...
3. GET /admin/integrations/settings — get current settings
4. PATCH /admin/integrations/settings — update settings
5. POST /admin/integrations/test-webhook — send test event
...
1. Bash script that:
   - Dumps all PostgreSQL databases (platform, hospitality, and future ones)
   - Backs up Redis data (or document cache-only)
   - Backs up n8n workflows (from n8n API or volume)
   ...
   - Logs to /var/log/cloudit-backup.log
2. Make executable
3. Add to docker-compose as cron job or document host cron setup
...
1. Placeholder script using rclone/rsync to sync /opt/backups/local/ to Hetzner Storage Box
2. Read credentials from env vars
3. Log sync operations
4. Mark as optional/future feature
...
After implementation:
1. Test backup script manually if possible.
...
**Health Checks**:
1. Platform API: enhance /health to check database, Redis, disk space, memory usage
2. Hospitality API: same enhanced health check
3. Add healthcheck directives to Docker Compose with curl to /health

**Logging**:
1. Add structured JSON logging to both APIs using NestJS Logger or pino
...
5. Add log rotation in Docker Compose: max-size 10m, max-file 3

**Error Handling**:
1. Global exception filter in both APIs:
...
3. Add interceptors for:
   - Response transformation (wrap in { success, data, error })
...
5. Verify all passwords are hashed with bcrypt
6. Add API key authentication for service-to-service (optional, document)
...
1. Create script that:
   ...
   - Alerts if usage > 80% (logs to file)
   - Lists largest directories in /opt and /var/lib/docker
2. Add to cron

**Docker Compose**:
1. Add resource limits to all services (for 8GB RAM):
   - postgres: 1GB, redis: 512MB
   - platform-api: 1GB, hospitality-api: 1GB
   - platform-web: 512MB, hospitality-web: 512MB
   - n8n: 1GB, traefik: 256MB, uptime-kuma: 512MB
2. Total should stay under 6GB

**Documentation**:
1. Update `docs/security.md` with all measures
...
1. Improve deployment workflow:
   - Add build caching for Docker layers
   - Add health checks after deployment
   - Add rollback step if health checks fail
   - Add notification step (log to file for now)
2. Add workflow for PR checks:
   - Lint, Type check, Unit tests (placeholder), Build check
...
3. `maintenance.sh` — enable/disable maintenance mode
...
1. Common issues and solutions:
   - Database connection failure, Redis failure, Disk full
   - Memory exhaustion, SSL certificate expiry, Deployment failure
2. Step-by-step recovery procedures
3. Contact/escalation info (placeholder)

**Complete Documentation Review**:
1. Ensure all docs are up to date:
   - deployment-guide.md, backup-restore.md, security.md
   - new-app-guide.md, ai-automation-setup.md
2. Add table of contents to main README.md
3. Add architecture diagram description (ASCII art)
...
### Sprint 7 Goal
Prepare structure for OrbitOne, TouchOrbit HR, and future SaaS apps without fully building them.
...
1. Create `apps/orbitone-api/`:
   - Copy minimal structure from `apps/hospitality-api`
   - NestJS setup with Prisma
   - Separate database: `orbitone`
   - Auth middleware (reuse from platform)
   - Health check endpoint
   - `.env.example`, `Dockerfile`, `prisma/schema.prisma` (empty schema with User/Organization relation)
   - Port: 3003
...
3. Add to docker-compose with Traefik labels:
   - `orbitone.cloudit.lk`, `api-orbitone.cloudit.lk`
...
2. Create `apps/touchorbit-web/`:
   ...
   - Port: 3006
3. Add to docker-compose with Traefik labels:
   - `touchorbit.cloudit.lk`, `api-touchorbit.cloudit.lk`

**Database Setup**:
1. Ensure PostgreSQL creates `orbitone` and `touchorbit` databases on startup
...
1. In `apps/orbitone-api/prisma/schema.prisma`, add example models:
   - Card, CardTemplate, CardView (analytics)
...
1. Document how Traefik routes work:
   - Host rule matching, Path prefix matching, Middleware usage
2. Example for adding a new subdomain:
   - DNS record in Cloudflare
   - Docker Compose labels
   - SSL certificate auto-generation
3. Example for path-based routing

**Shared Package Usage** (`docs/shared-packages.md`):
1. How to use `@cloudit/ui` in a new app
2. How to use `@cloudit/auth` for authentication
3. How to use `@cloudit/database` for Prisma client
...
1. Text-based description of full architecture:
...
2. Check no real secrets are committed
...
After implementation:
1. Do final `git status` to ensure nothing is uncommitted.
...
1. **One task at a time.** Kimi Claw reads the CURRENT TASK tracker and executes only that task.
2. **Wait for confirmation.** Do not proceed until Kimi Claw says "Task X.Y done".
3. **If Kimi Claw crashes:** Restart Kimi Claw, give it the Master Prompt, and say "Continue from Task X.Y".
