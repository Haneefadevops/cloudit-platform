...
const lifecycleStageSchema = z.enum(["new", "contacted", "qualified", "meeting", "proposal", "customer", "lost", "archived"]);
...
export const customerInputSchema = z.object({
  ...
  expectedCloseDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).nullable().optional().or(z.literal("")),
  outcome: outcomeSchema.optional(),
  closedReason: z.string().trim().max(1000).nullable().optional().or(z.literal("")),
...
export const customerActivityInputSchema = z.object({
  type: z.enum(["note", "call", "email", "meeting", "sms", "whatsapp", "other"]),
...
export const bulkActionInputSchema = z.object({
  ...
  action: z.enum(["delete", "assign", "set_stage", "set_lifecycle", "set_priority", "set_outcome"]),
...
export type CustomerLifecycleInput = z.infer<typeof customerLifecycleInputSchema>;
