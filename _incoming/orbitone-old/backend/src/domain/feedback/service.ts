...
function mapFeedbackRequest(row: Record<string, unknown>): FeedbackRequest {
  return {
    id: row.id as string,
...
export async function listFeedbackRequests(context: FeedbackContext, customerId: string): Promise<FeedbackRequest[]> {
  const result = await pool.query(
    `SELECT f.*
     FROM feedback_requests f
     JOIN customers c ON c.id = f.customer_id
     WHERE f.customer_id = $3 AND (${ownershipClause("c")})
     ORDER BY f.created_at DESC`,
    [context.organizationId, context.userId, customerId]
  );
  return result.rows.map(mapFeedbackRequest);
}
  ...
  try {
    ...
    if (input.bookingId) {
      ...
      if (bookingResult.rowCount === 0) {
        await client.query("ROLLBACK");
        throw new FeedbackError("Booking not found or does not belong to this customer.");
      }
    }
    ...
    await client.query(
      ...
      [customerId, context.userId, "Feedback request created", `Channel: ${channel}`]
    );

    await client.query("COMMIT");
    return mapFeedbackRequest(result.rows[0]);
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
  } finally {
...
export async function getFeedbackByToken(token: string): Promise<FeedbackTokenInfo | null> {
  const result = await pool.query(
  ...
  if (!profileId && row.booking_id) {
    const bookingResult = await pool.query("SELECT owner_user_id FROM bookings WHERE id = $1", [row.booking_id]);
    if (bookingResult.rowCount && bookingResult.rows[0].owner_user_id) {
      const profileResult = await pool.query("SELECT id FROM profiles WHERE user_id = $1 LIMIT 1", [
  ...
  if (!profileId) {
    ...
    const profileResult = await pool.query("SELECT id FROM profiles WHERE user_id = $1 LIMIT 1", [ownerId]);
    if (profileResult.rowCount) {
      profileId = profileResult.rows[0].id;
    }
  }
...
export async function recordFeedbackRating(token: string, ratingId: string): Promise<boolean> {
  const result = await pool.query(
...
export async function markFeedbackSent(token: string): Promise<boolean> {
  const result = await pool.query(
    `UPDATE feedback_requests
