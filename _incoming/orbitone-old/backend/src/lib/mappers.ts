...
export function mapOrganization(row: Record<string, unknown>): Organization {
  return {
    ...
    subscriptionRenewalAt: row.subscription_renewal_at ? (row.subscription_renewal_at as Date).toISOString() : null,
...
export function mapMeetingType(row: Record<string, unknown>): MeetingType {
  return {
    ...
    maxBookingsPerDay: row.max_bookings_per_day === null || row.max_bookings_per_day === undefined
      ? null
...
export function mapAvailabilityRule(row: Record<string, unknown>): AvailabilityRule {
  return {
    id: row.id as string,
    ownerUserId: row.owner_user_id as string,
...
export function mapAvailabilityException(row: Record<string, unknown>): AvailabilityException {
  return {
    id: row.id as string,
    ownerUserId: row.owner_user_id as string,
...
export function mapBooking(row: Record<string, unknown>): Booking {
  return {
    ...
    externalProvider: (row.external_provider as Booking["externalProvider"]) ?? null,
...
export function mapCustomer(row: Record<string, unknown>): Customer {
  return {
    ...
    lifecycleStage: (row.lifecycle_stage as Customer["lifecycleStage"]) ?? "new",
    ...
    expectedCloseDate: row.expected_close_date ? (row.expected_close_date as Date).toISOString().split("T")[0] : null,
...
export function mapCustomerActivity(row: Record<string, unknown>): CustomerActivity {
  return {
    id: row.id as string,
    customerId: row.customer_id as string,
    createdByUserId: row.created_by_user_id as string,
    activityTypeDefinitionId: (row.activity_type_definition_id as string | null) ?? null,
...
export function mapCustomerFollowUp(row: Record<string, unknown>): CustomerFollowUp {
  return {
    id: row.id as string,
    customerId: row.customer_id as string,
    createdByUserId: row.created_by_user_id as string,
...
export function mapCustomerRating(row: Record<string, unknown>): CustomerRating {
  return {
    id: row.id as string,
