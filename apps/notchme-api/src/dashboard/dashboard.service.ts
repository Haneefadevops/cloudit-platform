...
import { mapBookingWithJoins } from "../common/lib/mappers";
...
export class DashboardService {
  ...
  async getDashboardSummary(user: AuthContext): Promise<DashboardSummary> {
    ...
    const [profileMetrics, usage, crmSummary, upcomingBookings] =
      await Promise.all([
        profile
          ? this.analyticsService.getProfileMetrics(profile.id)
          : Promise.resolve(this.getEmptyProfileMetrics()),
        profile
          ? this.analyticsService.getUsageSummary(
              profile.id,
              this.planService.getPlanLimit(
                this.planService.getUserEffectivePlan(user),
              ).maxBookingsPerWeek,
            )
          : Promise.resolve(this.getEmptyUsageSummary()),
  ...
  // TODO: cross-domain â€” replace with CRM service call once customers domain is ported.
  private getCRMSummary(_context: {
    userId: string;
  ...
  private getEmptyProfileMetrics() {
    return {
      profileViews: 0,
      qrScans: 0,
      vcardDownloads: 0,
