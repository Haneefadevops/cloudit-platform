...
export const authCookieName = "notchme_session";
const legacyAuthCookieName = "orbitone_session";
...
export class SessionService {
  ...
  clearSessionCookie(res: Response): void {
    ...
    // Compatibility cutover: clear an NotchMe cookie whenever a session ends.
    res.clearCookie(legacyAuthCookieName, {
      httpOnly: true,
      sameSite: "lax",
      secure: this.isProduction(),
      path: "/",
  ...
  async revokeSessionByCookie(req: Request, res: Response): Promise<void> {
    const token =
      ...
      this.getCookie(req.headers.cookie, legacyAuthCookieName);
    if (token) {
      try {
