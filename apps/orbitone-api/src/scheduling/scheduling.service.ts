...
import type { Plan } from "../common/contracts/orbitone.v2";
import type {
  MeetingTypeInput,
...
export class SchedulingService {
  ...
  async getMeetingTypes(ownerUserId: string) {
  ...
  async createMeetingType(ownerUserId: string, input: MeetingTypeInput) {
    const slug = input.slug ?? this.slugService.slugify(input.title);
  ...
  async updateMeetingType(
    ...
    if (input.locationType !== undefined)
      addField("location_type", input.locationType);
    if (input.locationValue !== undefined)
      addField("location_value", input.locationValue);
    if (input.bufferBeforeMinutes !== undefined)
      addField("buffer_before_minutes", input.bufferBeforeMinutes);
    if (input.bufferAfterMinutes !== undefined)
      addField("buffer_after_minutes", input.bufferAfterMinutes);
    if (input.minNoticeMinutes !== undefined)
      addField("min_notice_minutes", input.minNoticeMinutes);
    if (input.bookingWindowDays !== undefined)
      addField("booking_window_days", input.bookingWindowDays);
    if (input.maxBookingsPerDay !== undefined)
      addField("max_bookings_per_day", input.maxBookingsPerDay);
    if (input.requiresApproval !== undefined)
      addField("requires_approval", input.requiresApproval);
  ...
  async deleteMeetingType(ownerUserId: string, id: string) {
    const result = await this.databaseService.query(
  ...
  async getAvailability(ownerUserId: string) {
  ...
  async updateAvailability(
    ...
    try {
      ...
      await client.query(
        "DELETE FROM availability_rules WHERE owner_user_id = $1",
        [ownerUserId],
      );
      await client.query(
        "DELETE FROM availability_exceptions WHERE owner_user_id = $1",
        [ownerUserId],
      );
      ...
      return this.getAvailability(ownerUserId);
    } catch (error) {
  ...
  async getBookings(ownerUserId: string) {
    const result = await this.databaseService.query(
      `SELECT
        b.*,
  ...
  async getBooking(ownerUserId: string, id: string) {
    const result = await this.databaseService.query(
      `SELECT
        b.*,
        mt.slug AS meeting_type_slug, mt.title AS meeting_type_title, mt.description AS meeting_type_description,
  ...
  async cancelBooking(ownerUserId: string, id: string, reason?: string) {
    const client = await this.databaseService.connect();
    try {
      await client.query("BEGIN");
      ...
      await client.query("COMMIT");
      return this.getBooking(ownerUserId, id);
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
    } finally {
  ...
  async approveBooking(ownerUserId: string, id: string) {
    const client = await this.databaseService.connect();
    try {
      await client.query("BEGIN");
      const result = await client.query(
      ...
      if (booking.customer_id) {
        await this.customersService.createBookingMeetingActivity(
          booking.id,
          booking.customer_id,
          ownerUserId,
  ...
  async declineBooking(ownerUserId: string, id: string, reason?: string) {
    const client = await this.databaseService.connect();
    try {
      await client.query("BEGIN");
      const result = await client.query(
  ...
  async getPublicBookingContext(profileSlug: string, meetingTypeSlug?: string) {
    ...
    const meetingTypesResult = await this.databaseService.query(
      meetingTypesQuery,
      params,
    );
  ...
  async getPublicBookingProfile(profileSlug: string) {
    const profileResult = await this.databaseService.query(
      "SELECT * FROM profiles WHERE slug = $1 AND is_published = true",
      [profileSlug],
    );
  ...
  async generateBookingSlots(options: {
    ownerUserId: string;
    ...
    const bufferBeforeMinutes = Number(
      options.meetingType.buffer_before_minutes ?? 0,
    );
    const bufferAfterMinutes = Number(
      options.meetingType.buffer_after_minutes ?? 0,
    );
    const minNoticeMinutes = Number(
      options.meetingType.min_notice_minutes ?? 0,
    );
    const bookingWindowDays = Number(
      options.meetingType.booking_window_days ?? 30,
    );
    ...
    for (const exception of exceptionsResult.rows) {
      const key =
        exception.exception_date instanceof Date
          ? (exception.exception_date as Date).toISOString().slice(0, 10)
          : String(exception.exception_date as string).slice(0, 10);
      exceptionsByDate.set(key, [
    ...
    while (currentDate <= endDate) {
      ...
      if (
        ...
        (bookingCountByDate.get(currentDate) ?? 0) >= maxBookingsPerDay
      ) {
      ...
      for (const interval of intervals) {
        for (
          ...
          const start = zonedDateTimeToUtc(
            currentDate,
            formatMinutesAsTime(slotStartMinutes),
            options.timezone,
          );
          const end = new Date(start.getTime() + durationMinutes * 60 * 1000);
          const candidateStart = new Date(
            start.getTime() - bufferBeforeMinutes * 60 * 1000,
          );
          const candidateEnd = new Date(
            end.getTime() + bufferAfterMinutes * 60 * 1000,
          );
  ...
  async getBookingSlots(
    ...
    const context = await this.getPublicBookingContext(
      profileSlug,
      meetingTypeSlug,
    );
    ...
    const slots = await this.generateBookingSlots({
  ...
  async getProfilePlan(profileUserId: string): Promise<Plan> {
    ...
    if (user.organization_id) {
      const orgResult = await this.databaseService.query(
        "SELECT plan FROM organizations WHERE id = $1",
        [user.organization_id],
      );
      const orgPlan = orgResult.rows[0]?.plan;
      if (orgPlan && this.isValidPlan(orgPlan)) {
        return orgPlan as Plan;
      }
    }

    if (this.isValidPlan(user.plan)) {
  ...
  async getWeeklyBookingCount(profileId: string) {
    const now = new Date();
  ...
  async incrementWeeklyBookingCount(profileId: string) {
    const now = new Date();
    const year = now.getFullYear();
    const week = getWeekNumber(now);
  ...
  async createPublicBooking(
    ...
    const requestedEnd = new Date(
      requestedStart.getTime() +
        Number(meetingType.duration_minutes) * 60 * 1000,
    );
    ...
    const plan = await this.getProfilePlan(ownerUserId);
    const limit = this.planService.getPlanLimit(plan);
    if (limit.maxBookingsPerWeek !== null) {
      const currentCount = await this.getWeeklyBookingCount(profileId);
    ...
    try {
      ...
      const matchingSlot = slots.find(
        (slot) => slot.startAt === requestedStart.toISOString(),
      );
      if (!matchingSlot) {
        await client.query("ROLLBACK");
      ...
      const customerId = await this.customersService.upsertCustomerFromBooking(
        {
          id: ownerUserId,
      ...
      const status =
        meetingType.requires_approval === true ? "pending" : "confirmed";
      ...
      await client.query(
        ...
        [
          ...
          ownerUserId,
          JSON.stringify({ source: input.source ?? "profile" }),
        ],
      );
      ...
      await client.query(
        ...
        [
          bookingResult.rows[0].id,
          this.hashGuestToken(rescheduleToken),
          input.guestEmail,
          tokenExpiresAt,
          this.hashGuestToken(cancelToken),
        ],
      );

      if (limit.maxBookingsPerWeek !== null) {
        await this.incrementWeeklyBookingCount(profileId);
      }
      ...
      const fullResult = await this.databaseService.query(
        `SELECT
