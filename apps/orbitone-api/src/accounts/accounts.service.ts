...
export class AccountsService {
  ...
  private readonly lifecycleStages: AccountLifecycleStage[] = [
    "prospect",
    "qualified",
    "customer",
    "churned",
  ...
  private mapAccount(row: Record<string, unknown>): Account {
    return {
      id: row.id as string,
  ...
  private mapCustomer(row: Record<string, unknown>): Customer {
    return {
      ...
      expectedCloseDate: row.expected_close_date
        ? (row.expected_close_date as Date).toISOString().slice(0, 10)
        : null,
  ...
  async listAccounts(context: AccountContext): Promise<Account[]> {
  ...
  async createAccount(
    ...
    const stage: AccountLifecycleStage =
      input.lifecycleStage &&
      this.lifecycleStages.includes(input.lifecycleStage)
        ? input.lifecycleStage
        : "prospect";
  ...
  async getAccountWithContacts(
    ...
    const account = await this.getAccount(context, id);
    if (!account) return null;
    const contactsResult = await this.databaseService.query(
      `SELECT * FROM customers WHERE account_id = $1 ORDER BY full_name ASC`,
      [id],
    );
    return {
      account,
      contacts: contactsResult.rows.map((row) => this.mapCustomer(row)),
    };
  }
  ...
  async updateAccount(
    ...
    const existing = await this.getAccount(context, id);
    ...
    if (input.slug !== undefined) {
      const slug =
        input.slug.trim() || this.slugify(input.name ?? existing.name);
  ...
  async deleteAccount(context: AccountContext, id: string): Promise<boolean> {
    const result = await this.databaseService.query(
  ...
  async listDirectory(
    ...
    if (options.search) {
      conditions.push(
        `(name ILIKE $${idx} OR industry ILIKE $${idx} OR website ILIKE $${idx})`,
      );
  ...
  async getPublicAccountBySlug(slug: string): Promise<Account | null> {
    const result = await this.databaseService.query(
  ...
  async requestConnection(
    ...
    const fromResult = await this.databaseService.query(
      "SELECT id, organization_id FROM business_accounts WHERE id = $1",
      [fromAccountId],
    );
    if (fromResult.rowCount === 0)
      throw new AccountError("From account not found.", 404);
    if (fromResult.rows[0].organization_id !== context.organizationId) {
      throw new AccountError(
        "You can only connect from an account your organization owns.",
      );
    }

    const toResult = await this.databaseService.query(
      "SELECT id, is_public FROM business_accounts WHERE id = $1",
      [toAccountId],
    );
    if (toResult.rowCount === 0)
      throw new AccountError("Target account not found.", 404);
    ...
    if (existing.rowCount && existing.rowCount > 0) {
      ...
      const otherId =
        row.from_account_id === fromAccountId
          ? row.to_account_id
          : row.from_account_id;
      const otherAccount = await this.getPublicAccountById(otherId);
      if (!otherAccount)
        throw new AccountError("Target account not found.", 404);
      return this.mapConnection(row, otherAccount);
    }
    ...
    const otherAccount = await this.getPublicAccountById(toAccountId);
    if (!otherAccount) throw new AccountError("Target account not found.", 404);
    return this.mapConnection(result.rows[0], otherAccount);
  }

  async getPublicAccountById(id: string): Promise<Account | null> {
    const result = await this.databaseService.query(
  ...
  async listConnections(
    ...
    const connections = await Promise.all(
      result.rows.map(async (row) => {
        ...
        const otherAccount = await this.getPublicAccountById(otherId);
        if (!otherAccount)
          throw new AccountError("Connected account not found.", 404);
        return this.mapConnection(row, otherAccount);
      }),
    );
    return connections;
  ...
  async respondConnection(
    ...
    const connectionResult = await this.databaseService.query(
    ...
    if (connection.organization_id !== context.organizationId) {
      throw new AccountError(
        "You can only respond to requests sent to your organization's account.",
      );
    }
