...
export class DocumentsService {
  ...
  async listDocuments(
    ...
    const result = await this.databaseService.query(
      `SELECT d.*
       ...
       WHERE d.customer_id = $3 AND (${this.ownershipClause("c")})
  ...
  async getDocument(
    ...
    const result = await this.databaseService.query(
      `SELECT d.*
       FROM documents d
       JOIN customers c ON c.id = d.customer_id
       WHERE d.id = $3 AND (${this.ownershipClause("c")})`,
  ...
  async updateDocumentStatus(
    ...
    try {
      ...
      const doc = this.mapDocument(updateResult.rows[0]);
      await client.query(
        `INSERT INTO customer_activities (customer_id, created_by_user_id, type, title, body, occurred_at)
         VALUES ($1, $2, 'other', $3, $4, now())`,
        [
  ...
  async generateDocumentPDF(document: Document): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      docPdf
        .fontSize(12)
        .fillColor("#555")
        .text(`Type: ${document.type.toUpperCase()}`);
      ...
      if (Array.isArray(data.items) && data.items.length > 0) {
        ...
        if (taxRate > 0) {
          docPdf.text(`Tax (${taxRate}%): LKR ${tax.toFixed(2)}`, {
            align: "right",
          });
        }
        docPdf
          .fontSize(14)
          .text(`Total: LKR ${grandTotal.toFixed(2)}`, { align: "right" });
      }
