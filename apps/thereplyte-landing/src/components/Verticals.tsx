        ...
        <div className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {verticals.map((v, i) => (
            <Reveal key={v.name} delay={(i % 3) * 0.08}>
              <div className="glass group h-full rounded-2xl px-6 py-5 transition-all duration-300 hover:border-white/20 hover:shadow-[0_0_32px_-10px_rgba(0,216,199,0.4)]">
                <div className="flex items-center gap-2.5">
                  <span className="h-2 w-2 rounded-full bg-brand-gradient" />
                  <h3 className="font-semibold text-white">{v.name}</h3>
                </div>
                <p className="mt-2 text-sm leading-relaxed text-slate-400">{v.use}</p>
              </div>
            </Reveal>
          ))}
        </div>
