...
export default function PublicProfilePage() {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      await navigator.share({ title: profile?.fullName ?? "NotchMe", url: publicUrl });
    } else {
    ...
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-background to-surface px-4 py-8 sm:py-12">
      <div className="mx-auto w-full max-w-xl animate-fade-in-up space-y-6">
        ...
        <p className="pb-6 text-center text-xs text-muted">
          Powered by <span className="font-medium text-foreground">NotchMe</span>
        </p>
      </div>
    </div>
  );
...
function ActionButton({
  ...
  const className = cn(
    "flex flex-1 flex-col items-center gap-1 rounded-lg px-2 py-2 text-xs font-medium text-muted transition-colors hover:bg-surface-elevated hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"
  );
